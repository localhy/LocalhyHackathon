...
const AuthPage = () => {
  ...
  const [signUpData, setSignUpData] = useState({
    ...
    userType: '',
    newsletter: true,
    referredBy: '' // Add referredBy to state
  })
  ...
  useEffect(() => {
    // Read 'ref' parameter from URL on component mount
    const ref = searchParams.get('ref');
    if (ref) {
      setSignUpData(prev => ({ ...prev, referredBy: ref }));
    }
  }, [searchParams]);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await signUp(signUpData.email, signUpData.password, {
        ...
        referred_by: signUpData.referredBy // Pass referredBy
      })
          ...
          <div className="bg-gray-50 rounded-xl p-4 sm:p-6 lg:p-8 shadow-lg">
            ...
            <div className={`transition-all duration-300 ${isLogin ? 'hidden' : 'block'}`}>
              ...
              <form onSubmit={handleSignUp} className="space-y-3 sm:space-y-4">
                ...
                {signUpData.referredBy && ( // Display referred by if present
                  <div>
                    <label 
                      htmlFor="referred-by" 
                      className="block text-sm font-medium text-gray-700 mb-2"
