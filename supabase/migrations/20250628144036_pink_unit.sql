/*
  ...
  1. New Tables
    - `community_posts` - Stores user posts for the community feed
      - `id` (uuid, primary key)
    ...
    - `community_post_likes` - Tracks likes on community posts
      - `id` (uuid, primary key)
  ...
  3. Functions
    - Add functions to manage likes and comments counts
*/
...
CREATE INDEX IF NOT EXISTS idx_community_posts_location ON public.community_posts(location) WHERE location IS NOT NULL;
...
-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_community_post_likes_community_post_id ON public.community_post_likes(community_post_id);
