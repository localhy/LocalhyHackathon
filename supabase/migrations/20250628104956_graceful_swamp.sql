/*
  ...
  1. New Features
    - Add `free_credits` column to user_profiles for promotional credits
    - Add credit transfer functionality between users
    - Update referral job posting to use free credits first
    - Enhanced credit purchase with bonus free credits

  2. Transaction Types
    - Add `credit_transfer_sent` and `credit_transfer_received` transaction types

  3. Functions
    - `transfer_user_credits`: Transfer cash credits between users
    - Updated `create_referral_job_with_payment`: Use free credits first, then cash credits
    - Updated `add_credits_to_user`: Handle bonus free credits for certain packages

  4. Security
    - Free credits can only be used for platform activities (referral job posting)
    - Free credits cannot be transferred or withdrawn
    - Cash credits are used after free credits are exhausted
*/
...
DROP FUNCTION IF EXISTS transfer_user_credits(UUID, TEXT, INTEGER);
DROP FUNCTION IF EXISTS create_referral_job_with_payment(UUID, TEXT, TEXT, TEXT, NUMERIC, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER);
DROP FUNCTION IF EXISTS add_credits_to_user(UUID, INTEGER, NUMERIC, TEXT, TEXT, TEXT);
...
-- Create referral job with payment function (uses free credits first)
...
-- Add credits to user function (handles bonus free credits)
...
GRANT EXECUTE ON FUNCTION transfer_user_credits(UUID, TEXT, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION create_referral_job_with_payment(UUID, TEXT, TEXT, TEXT, NUMERIC, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION add_credits_to_user(UUID, INTEGER, NUMERIC, TEXT, TEXT, TEXT) TO authenticated;